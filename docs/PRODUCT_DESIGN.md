...
**主控端与宿主凭据（定案 2026-08-16）**：主控端 = 完全远程控制（与 ToDesk/UU 同权）——官方界面内的全部功能（含宿主凭据/模型配置向导）都在设备密码授权的范围内，WhaleMaid 不额外限制、也不额外新增任何 key 输入面（官方前端调用点零改动）；第三方 key 一律只落宿主本地（ADR-013/TM-007），手机侧仅持久化"设备编号+密码"这一对可吊销设备凭据（应用私有存储），不持久化任何宿主 API key。
