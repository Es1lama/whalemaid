...
- **修复方案**：`scrypt`（PHC，Node/Rust 跨语言互验，参数 ln=14,r=8,p=1）加盐哈希存服务端；`/connect` 按 (IP, deviceId) **失败尝试限速**（5 次/分钟）+ 连续失败锁定（5 次锁 5 分钟）——**窗口预算只由失败消耗**：主控端逐请求签 grant（每个静态资源一次 /connect）是合法高频，成功验证证明口令已知、不增加猜测能力，故成功不占预算（架构修订 2026-08-15）；受控端注册时上报的密码哈希直接复用（服务端不碰明文密码）。
- **验收单测（已绿）**：错误密码 401；同哈希不同盐产出不同；失败消耗窗口预算（窗口内第 6 次失败被拒 429）；连续 5 次失败锁定 423、锁定期内正确密码也拒；**成功验证不消耗窗口预算（高频签 grant 不受限，回归测试）**；成功清零失败历史。
