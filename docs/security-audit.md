...
- **安装令牌（审计三轮#4 修订，2026-08-16）**：安装码不再是无限复用的静态共享秘密——改为**可消费安装令牌**：服务端只存 SHA-256 哈希（明文仅在签发时返回一次），每个令牌带使用上限（默认 1 次）与可选 TTL；注册成功即消耗。管理员经 `POST /_whalemaid/admin/install-tokens`（管理令牌认证）签发新令牌；`ADMIN_INSTALL_CODE` 环境变量仅作为首启种子（默认 1 次）。令牌耗尽/过期/未知 → 401。
...
- **我们现状（已修）**：服务端只持有 scrypt PHC；失败尝试限速与锁定已实现；成功验证不消耗窗口预算。
- **修复方案**：`scrypt`（PHC，Node/Rust 跨语言互验，参数 ln=14,r=8,p=1）加盐哈希存服务端；`/connect` 按 (IP, deviceId) **失败尝试限速**（5 次/分钟）+ 连续失败锁定（5 次锁 5 分钟）——**窗口预算只由失败消耗**：主控端逐请求签 grant（每个静态资源一次 /connect）是合法高频，成功验证证明口令已知、不增加猜测能力，故成功不占预算（架构修订 2026-08-15）；受控端注册时上报的密码哈希直接复用（服务端不碰明文密码）。scrypt 在短暂复制注册记录后移出全局 `Registry` 锁，放到有 8 槽上限的 blocking 池并发执行；验证后重新确认设备未吊销且 PHC 未轮换。首次成功再签随机 256-bit `sessionToken`（15min、绑定客户端 IP+设备、服务端仅内存、主控端仅进程内），后续逐请求凭令牌快速签单用途 grant，避免为每个静态资源重复慢哈希；密码轮换/设备吊销立即清除该设备会话令牌与 grant。
- **验收单测（已绿）**：错误密码 401；同哈希不同盐产出不同；失败消耗窗口预算（窗口内第 6 次失败被拒 429）；连续 5 次失败锁定 423、锁定期内正确密码也拒；**成功验证不消耗窗口预算（高频签 grant 不受限，回归测试）**；成功清零失败历史；Android 强停重启后首屏并发资源无需刷新即可完成渲染。
