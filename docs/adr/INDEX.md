...
| ADR-005 | 技术栈 | 中继/控制面 Rust；被控插件 Node/TS（三平台）；主控端 = 官方前端移植（React/TS）三壳 | DESIGN §3 |
| ADR-006 | 中继基底 | rathole（Rust，Apache-2.0）**sidecar 管理不重写** + 自研控制面；frp 备选 | DESIGN §2 |
| ADR-007 | ~~移动端 UI 全自研~~ **已废止** | 原"全自研、与 dsh-web-ui 零合作零复制（避免纠纷）"被 ADR-039 取代（官方前端 MIT 移植） | REQ-REJ-006 |
...
| ADR-019 | 直连模式 | 开源原生支持 IP 直连；官方中继是默认承载（ADR-042 起直连降级为高级/专家选项，默认 UX 无 IP） | DESIGN §5 |
...
| ADR-039 | 前端策略 v2 | **移植 > 复刻**：官方 DSH web 前端（MIT）移植 + 移动/桌面适配 + 原生壳打包（Capacitor Android/iOS、Electron PC、Web 三供，D-025 定案），同源调用原生 /api 经网关打到受控端 | DESIGN §3 |
