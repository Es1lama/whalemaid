...
- ◐ REQ-003 临时密码：中继核心已实现设备凭据签发/刷新/撤销、服务端 TTL、generation 原子单次消费、临时 session 与稳定错误码；受控插件已通过官方 sidebar slot 提供设备编号、生成/刷新/复制/倒计时/撤销 UI，且终态清除明文。Web/Electron 共用控制器已接 credential mode、进程内临时 session、稳定错误文案且临时设备不落 localStorage；Android/iOS 仍待完成，不得标 ✅；
